  ...
  <AppLayout>
        ...
        <div>
          ...
          <div v-if="selected" class="text-center mt-4">
            <div v-if="selected.stat">
              <div class="mb-1">
                プレイ数：{{ selected.stat.played }}
              </div>
              <div class="mb-1">
                パーフェクト数：{{ selected.stat.perfect }}
              </div>
              <div class="mb-1">
                最低WPM：{{ selected.stat.min_wpm }}
              </div>
              <div class="mb-1">
                最高WPM：{{ selected.stat.max_wpm }}
              </div>
              <div class="mb-1">
                平均WPM：{{ selected.stat.ave_wpm }}
              </div>
              <div class="mb-1">
                最低正答率：{{ selected.stat.min_accuracy }}%
              </div>
              <div class="mb-1">
                最高正答率：{{ selected.stat.max_accuracy }}%
              </div>
              <div class="mb-1">
                平均正答率：{{ selected.stat.ave_accuracy }}%
              </div>
              <div class="mb-1">
                最大連続ミス数：{{ selected.stat.max_miss_streak }}
              </div>
    ...
    <ContentFrame>
      ...
      <template #content>
        <div class="text-center mb-4">
          <div class="mt-1">
            WPM：{{ userStats.wpm.toFixed(2).replace('.00', '') }}
          </div>
          <div class="mt-1">
            最高WPM：{{ userStats.max_wpm }}
          </div>
          <div class="mt-1">
            正答率：{{ userStats.accuracy.toFixed(2).replace('.00', '') }}%
          </div>
          <div class="mt-1">
            打鍵数：{{ userStats.typed }}
          </div>
          <div class="mt-1">
            プレイ回数：{{ userStats.played }}
          </div>
          <div class="mt-1">
            プレイ時間：{{ formatSeconds(userStats.played_seconds) }}
          </div>
        </div>
      </template>
    </ContentFrame>
    <DangerButton class="mt-10 mx-auto shadow-xl" @click="resetAllConfirm = true">
      すべての統計をリセット
    </DangerButton>
    ...
    <AppModal v-if="resetConfirm" :show="resetConfirm" @close="resetConfirm = false">
      ...
      <div v-if="selected && selected.stat">
        ...
        <DangerButton class="mt-6 mx-auto" @click="reset">
          リセット
        </DangerButton>
      </div>
    </AppModal>
